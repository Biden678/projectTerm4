...
function Forbidden(props) {
  ...
  useEffect(() => {
    setTotalPages(Math.ceil(dataForbbiden.length / PAGE_SIZE));
  }, [dataForbbiden]);
  const displayedForbbidens = dataForbbiden.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);
  return (
    <div className='table-responsive mt-5'>
      ...
      <table className="table text-nowrap mb-0 align-middle">
        ...
        <tbody>
          {displayedForbbidens.length > 0 && displayedForbbidens.map((item, index) => {
