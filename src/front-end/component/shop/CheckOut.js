            ...
            for (let cart of carts) {
                try {
                    ...
                    if (response.status === 200) {
                        ...
                        const isOutOfStock = (cart?.type_unit === 1 && (
                            (cart?.option_unit === 1 && unitPrices.quantity_box < cart.quantity_unit) ||
                            (cart?.option_unit === 2 && unitPrices.quantity_pack < cart.quantity_unit) ||
                            (cart?.option_unit === 3 && unitPrices.quantity_pill < cart.quantity_unit)
                        )) || (cart?.type_unit === 2 && cart?.option_unit === 4 && unitPrices.quantity_other < cart.quantity_unit);

                        if (isOutOfStock) {
                            setIsValidOrder(false);
                            alert("Order không hợp lệ xin hãy kiểm tra lại giỏ hàng");
                            navigate('/cart');
                            return;
                        }
                    }
